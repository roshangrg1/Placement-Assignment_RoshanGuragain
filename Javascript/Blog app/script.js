...
const onPageLoad = async () => {
  ...
  data.map((element, index) => {
    ...
    container.insertAdjacentHTML(
      "afterbegin",
      `<div class=" bg-[#ffffff] w-[250px] min-h-[250px] flex flex-col justify-between  items-center  p-4 rounded-md drop-shadow-md">
      <h2 class="text-[blue] font-serif text-xl text-center">${element.title}</h2>
      <p class="text-center text-[#000]">${element.body}</p>
    </div>`
    );
  });
};
...
const addData = async (event) => {
  ...
  <div class="w-[250px] min-h-[250px] flex flex-col justify-between items-center bg-[#ffffff] p-4 rounded-md drop-shadow-md">
      <h2 class="text-[blue] font-serif text-xl text-center">${title}</h2>
